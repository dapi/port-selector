...
Чтобы видеть полную информацию о процессах (PID, имя процесса) для портов, принадлежащих другим пользователям при сканировании, запускайте через sudo. **Важно:** используйте флаг `-E` для сохранения переменных окружения, иначе конфиг будет создан в `/root/.config/`:

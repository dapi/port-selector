...
При **первом запуске** (когда директория конфигурации ещё не существует), `port-selector` автоматически выполняет тихое сканирование для обнаружения уже занятых портов в системе. Это гарантирует, что новые аллокации не будут конфликтовать с существующими сервисами.

Также можно запустить сканирование вручную в любое время:
...
Чтобы отключить авто-сканирование при первом запуске:
...
Options:
  ...
  --no-auto-scan    Отключить авто-сканирование при первом запуске
```

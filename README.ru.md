...
Назначайте несколько стабильных портов на директорию с помощью флага `--name`. Это полезно, когда один worktree нуждается в нескольких сервисах (например, веб-сервер, API, база данных):

```bash
# Выделить порты для разных сервисов в одной директории
...
# Повторно использовать именованную аллокацию — вернётся тот же порт
$ port-selector --name web
3010
...
**Как это работает:**
- Каждая пара `(директория, имя)` получает свой стабильный порт
- Имя по умолчанию — `"main"`, когда `--name` не указан
- Имя `"main"` показывается в `--list` только когда у директории есть другие именованные аллокации
- Порты, выделенные другим именам в той же директории, исключаются из выбора

Это идеально подходит для:
- **Микросервисов**: Каждый сервис в монорепе получает стабильный порт
- **Docker Compose**: Несколько контейнеров из одного проекта
- **Full-stack разработки**: Frontend, backend и база данных в одной директории
